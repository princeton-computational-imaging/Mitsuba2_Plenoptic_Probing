...
class TemporalPolarizedPlastic final : public BSDF<Float, Spectrum> {
public:
    MTS_IMPORT_BASE(BSDF, m_flags, m_components)
    MTS_IMPORT_TYPES(Texture, MicrofacetDistribution)

    TemporalPolarizedPlastic(const Properties &props) : Base(props) {
        m_diffuse_reflectance  = props.texture<Texture>("diffuse_reflectance",  .5f);

        a_diff = props.float_("a_diff", 0.5f);
        b_diff = props.float_("b_diff", 0.5f);
        c_diff = props.float_("c_diff", 0.5f);

        a_spec = props.float_("a_spec", 0.5f);
        b_spec = props.float_("b_spec", 0.5f);
        c_spec = props.float_("c_spec", 0.5f);

        render_at = props.float_("render_at", 0.f);
        speed_of_light = props.float_("speed_of_light", 10000.f);
    ...
    std::pair<BSDFSample3f, Spectrum> sample(const BSDFContext &ctx,
        ...
        // TODO apply the time dependence integral to the BSDF sample
        return { bs, result / bs.pdf & active };
    }

    Vector4f diffusion_vector(
      const Vector4f a,
      const Vector4f b,
      const Vector4f c,
      const Float time_of_flight,
      const float time
    ) const {
      auto time_component = -sqr(time - time_of_flight - b);
      time_component = time_component/(2 * sqr(c));
      time_component = exp(time_component);
      return a * time_component;
    }
    ...
    Spectrum eval(const BSDFContext &ctx, const SurfaceInteraction3f &si,
        ...
        if constexpr (is_polarized_v<Spectrum>) {
            float t = render_at;
            Float tof = si.time / speed_of_light;
            ...
            if (has_specular) {
                ...
                // TODO convert this D into a depolarization mueller matrix with a time
                // component?
                ...
                UnpolarizedSpectrum spec = m_specular_reflectance ? m_specular_reflectance->eval(si, active)
                                                                  : 1.f;
                // TODO multiply this by new depolarization mueller matrix
                Vector4f rho_d_t = diffusion_vector(a_spec, b_spec, c_spec, t, tof);
                auto Ys = mueller::depolarizer_vec(rho_d_t);
                result += spec * F * value * Ys;
            }
            ...
            if (has_diffuse) {
                ...
                Spectrum diff_refl = m_diffuse_reflectance->eval(si, active);
                Vector4f rho_d_t = diffusion_vector(a_diff, b_diff, c_diff, t, tof);
                Spectrum diff = diff_refl * mueller::depolarizer_vec(rho_d_t);
    ...
    Float pdf(const BSDFContext &ctx, const SurfaceInteraction3f &si,
        ...
        // TODO apply integral of time to probability
...
private:
    ...
    // Coefficients for diffuse temporal component.
    Vector4f a_diff;
    Vector4f b_diff;
    Vector4f c_diff;

    // Coefficients for specular temporal component.
    Vector4f a_spec;
    Vector4f b_spec;
    Vector4f c_spec;

    // What timestamp we would like to isolate for rendering.
    float render_at;
    // Speed of light to use when rendering time-gating
    float speed_of_light;
};

MTS_IMPLEMENT_CLASS_VARIANT(TemporalPolarizedPlastic, BSDF)
MTS_EXPORT_PLUGIN(TemporalPolarizedPlastic, "Polarized plastic")
