...
class TemporalGatedIntegrator : public MonteCarloIntegrator<Float, Spectrum> {
public:
    ...
    TemporalGatedIntegrator(const Properties &props) : Base(props) {
      min_t = props.float_("min_t", 0);
      // TODO maybe make this infinity or something?
      max_t = props.float_("max_t", 100000);

      light_speed = props.float_("light_speed", 1000);
      inv_light_speed = 1.f/light_speed;
    }
    ...
    std::pair<Spectrum, Mask> sample(const Scene *scene,
        ...
        Float time = si.t * inv_light_speed;
        ...
        for (int depth = 1;; ++depth) {
            ...
            if (likely(any_or<true>(active_e))) {
                ...
                Float end_time = time + ds.dist * inv_light_speed;
                // Check that the time falls within the gate
                active_e &= end_time <= max_t && end_time >= min_t;
            ...
            time += si_bsdf.t * inv_light_speed;
    ...
    std::string to_string() const override {
        return tfm::format("TemporalGatedIntegrator[\n"
            "  max_depth = %i,\n"
            "  rr_depth = %i\n"
    ...
    private:
      ScalarFloat min_t;
      ScalarFloat max_t;
      // arbitrary marker for speed of light
      ScalarFloat light_speed;
      ScalarFloat inv_light_speed;
};

MTS_IMPLEMENT_CLASS_VARIANT(TemporalGatedIntegrator, MonteCarloIntegrator)
MTS_EXPORT_PLUGIN(TemporalGatedIntegrator, "Path Tracer integrator");
