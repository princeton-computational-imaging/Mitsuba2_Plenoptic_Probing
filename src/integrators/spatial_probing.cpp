...
class SpatialIntegrator : public MonteCarloIntegrator<Float, Spectrum> {
public:
    ...
    SpatialIntegrator(const Properties &props) : Base(props) {
      m_delta = Point2f(
      ...
      // currently unused
      m_disparity = Point2f(
        props.float_("disparity_x", 0),
        props.float_("disparity_y", 0));

      // used to control depth-band width
      m_eps = Point2f(
      ...
      // TODO maybe make this infinity or something?
    ...
    std::pair<Spectrum, Mask> sample(const Scene *scene,
        ...
        Float time = si.t * m_inv_light_speed;
        ...
        for (int depth = 1;; ++depth) {
            ...
            // break if all rays have taken longer than the maximum threshold
            ...
            if (likely(any_or<true>(active_e))) {
                /*
                Point2f uv = position_sample * cs + sampler->next_2d(active_e) + m_disparity;
                uv = Point2f(fmod(uv.x(), Float(cs.x())), fmod(uv.y(), Float(cs.y())));
                */
                // Point2f uv = relevant(position_sample, sampler->next_2d(active_e),
                auto [ds, emitter_val] = scene->sample_emitter_direction(si,
                  sampler->next_2d(active_e), true, active_e);

                // time gating component
                Float end_time = time + ds.dist * m_inv_light_speed;
                // Check that the time falls within the gate
                active_e &= end_time <= m_max_t && end_time >= m_min_t;

                // todo can make this into a transformed relation before doing compare
                // in order to get non-linear effects
            ...
            time += si_bsdf.t * m_inv_light_speed;
    ...
    // Uses sample to compute a point that is relevant to near
    Point2f relevant(const Point2f near, const Point2f sample) const {
      auto rel = near + m_disparity;
      auto sample_delta = m_delta * (sample * 2.f - 1.f);
      return Point2f(
        wrap(wrap(rel.x()) + sample_delta.x()),
        wrap(wrap(rel.y()) + sample_delta.y()));
    }

    Float wrap(Float v) const {
      Float u = fmod(v, 1.f);
      return u >= 0.f ? u : 1.f + u;
    }
...
MTS_IMPLEMENT_CLASS_VARIANT(SpatialIntegrator, MonteCarloIntegrator)
MTS_EXPORT_PLUGIN(SpatialIntegrator, "Spatial Probing Integrator");
