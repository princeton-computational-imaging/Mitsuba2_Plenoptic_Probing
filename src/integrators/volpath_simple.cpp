...
public:
    ...
    VolumetricNullSimplePathIntegrator(const Properties &props) : Base(props) {
      m_delta = Point2f(
        clamp(props.float_("delta_x", 1), 0, 1),
        clamp(props.float_("delta_y", 1), 0, 1));

      m_disparity = Point2f(
        clamp(props.float_("disparity_x", 0), 0, 1),
        clamp(props.float_("disparity_y", 0), 0, 1));

      m_iso_y_min = clamp(props.float_("min_isolate_y", 0), 0, 1);
      m_iso_y_max = clamp(props.float_("max_isolate_y", 1), m_iso_y_min, 1);

      m_iso_emitter_x_min = clamp(props.float_("min_isolate_emitter_x", 0), 0, 1);
      m_iso_emitter_x_max = clamp(props.float_("max_isolate_emitter_x", 1),
                                  m_iso_emitter_x_min, 1);

      m_disparity = Point2f(
        props.float_("eps_x", 2.f * m_delta.x()),
        props.float_("eps_y", 2.f * m_delta.y()));

      m_invert_x = !props.bool_("invert_x", false);
      m_invert_y = !props.bool_("invert_y", false);

      m_min_t = props.float_("min_t", 0);
      m_max_t = props.float_("max_t", 100000);
      m_inv_light_speed = 1.f/props.float_("light_speed", 1000);
    }
    ...
    std::pair<Spectrum, Mask> sample(const Scene *scene,
                                     Sampler *sampler,
                                     const Sensor* /* sensor */,
                                     const Vector2f position_sample,
        ...
        if (position_sample.y() < m_iso_y_min || position_sample.y() > m_iso_y_max)
          return { Spectrum(0), valid_ray };
        ...
        for (int bounce = 0;; ++bounce) {
            ...
            if (none(active) || all(exceeded_max_depth)) break;

            if (all(time > m_max_t)) break;
            ...
            if (any_or<true>(active_medium)) {
                ...
                if (any_or<true>(intersect)) {
                    ...
                    masked(time, intersect) += si.t * m_inv_light_speed;
                }
            ...
            if (any_or<true>(act_medium_scatter)) {
                ...
                if (any_or<true>(active_e)) {
                    auto [ds, emitted_val] =
                      scene->sample_emitter_direction(mi, sampler->next_2d(active_e),
                                                      false, active_e);
                    Float end_time = time + ds.dist * m_inv_light_speed;
                    active_e &= end_time <= m_max_t && end_time >= m_min_t;

                    // spatial probing
                    Point2f delta = ds.uv - (position_sample + m_disparity);
                    auto accept_x = delta.x() < m_delta.x() && delta.x() > m_delta.x() - m_eps.x();
                    active_e &= m_invert_x ^ accept_x;
                    auto accept_y = delta.y() < m_delta.y() && delta.y() > m_delta.y() - m_eps.y();
                    active_e &= m_invert_y ^ accept_y;

                    if (ds.uv.x() < m_iso_emitter_x_min || ds.uv.x() > m_iso_emitter_x_max)
                      emitted_val = Spectrum(0.);
                    //
                    ...
                    if (any_or<true>(active_e)) {
                        ...
                        nee_ray.mint = 0.f;
                        auto emitted =
                          evaluate_direct_light(mi, scene, sampler, medium, nee_ray, true, si,
                                                emitted_val, ds.dist, channel, active_e).first;
                        Float phase_val = phase->eval(phase_ctx, mi, ds.d, active_e);
                        // masked(result, active_e) += throughput * phase_val * emitted / ds.pdf;
                        masked(result, active_e) +=
                          phase->polarization(phase_ctx,
                                              throughput * phase_val * emitted/ds.pdf,
                                              mi,
                                              ds.d,
                                              active_e);
                    }
                ...
                // all further interactions will be polarized as if it hit the media here
                masked(throughput, act_medium_scatter) =
                  phase->polarization(phase_ctx, throughput, mi, wo, act_medium_scatter);
            }
            ...
            if (any_or<true>(active_surface)) {
                ...
                if (likely(any_or<true>(active_e))) {
                    auto [ds, emitted_val] = scene->sample_emitter_direction(si,
                      sampler->next_2d(active_e), false, active_e);
                    // time gating
                    Float end_time = time + ds.dist * m_inv_light_speed;
                    active_e &= end_time <= m_max_t && end_time >= m_min_t;
                    ...
                    if (any_or<true>(active_e)) {
                        ...
                        auto emitted =
                          evaluate_direct_light(si, scene, sampler, medium, nee_ray, true, si,
                                                emitted_val, ds.dist, channel, active_e).first;
                        ...
                        result[active_e] += throughput * bsdf_val * mis * emitted/ds.pdf;
                    }
                }
                ...
                if (any_or<true>(intersect2)) {
                    ...
                    time += si_new.t * m_inv_light_speed;
                 }
                needs_intersection &= !intersect2;

                auto [emitted, emitter_pdf] =
                  evaluate_direct_light(si, scene, sampler, medium, ray, false, si_new,
                                        Spectrum(0.0), -1.f, channel, add_emitter);
    ...
    evaluate_direct_light(const Interaction3f &ref_interaction, const Scene *scene,
                          ...
                          Spectrum emitter_val, Float dist, UInt32 channel, Mask active) const {
...
protected:
    ...
    ScalarPoint2f m_delta;
    ScalarPoint2f m_disparity;
    ScalarPoint2f m_eps;

    bool m_invert_x;
    bool m_invert_y;

    ScalarFloat m_iso_y_min;
    ScalarFloat m_iso_y_max;

    ScalarFloat m_iso_emitter_x_min;
    ScalarFloat m_iso_emitter_x_max;

    ScalarFloat m_min_t;
    ScalarFloat m_max_t;
    ScalarFloat m_inv_light_speed;
};
