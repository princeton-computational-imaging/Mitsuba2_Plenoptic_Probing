...
class CodedOrthographicCamera final : public ProjectiveCamera<Float, Spectrum> {
public:
    ...
    CodedOrthographicCamera(const Properties &props) : Base(props) {
        code = props.texture<Texture>("code", Texture::D65(1.f));

        if (m_world_transform->has_scale())
            Throw("Scale factors in the camera-to-world transformation are not allowed!");
        m_scale = props.float_("scale", 2.0f);
    ...
    void update_camera_transforms() {
        ...
        */
        ...
        m_camera_to_sample =
          ...
          ScalarTransform4f::translate(ScalarVector3f(-0.5f, -0.5f/m_aspect, 0.f)) *
          // [0, 1], [0, 1], ...
          ScalarTransform4f::scale(
            ScalarVector3f(1.f/m_resolution.x(), 1.f/m_resolution.y(), 1.f));
            // [0, w], [0, h], [0, 1],
        ...
        m_sample_to_camera = m_camera_to_sample.inverse();
          */
        m_sample_to_camera =
          ScalarTransform4f::orthographic(m_near_clip, m_far_clip).inverse() *
          ScalarTransform4f::scale(ScalarVector3f(-1.f * (m_resolution.x() * m_scale),
            -1.f * (m_resolution.y() * m_aspect * m_scale), 1.f)) *
          ScalarTransform4f::translate(ScalarVector3f(-0.5f, -0.5f/m_aspect, 0.f));
        ...
        // TODO might need to add a clip transform here?
    }
    ...
    sample_ray_differential(Float time, Float wavelength_sample, const Point2f &position_sample,
        ...
        Point3f near_p = m_sample_to_camera.transform_affine(
                         Point3f(
                           position_sample.x(),
        ...
        ray.d = trafo * Vector3f(0, 0, 1);
        ray.update();
        /*
        if (any(position_sample < 0.25))
          std::cout << ray << std::endl;
        */

        ray.o_x = trafo * (near_p + m_dx);
        ray.o_y = trafo * (near_p + m_dy);
        ray.d_x = ray.d_y = ray.d;
        ray.has_differentials = true;

        auto si = zero<SurfaceInteraction3f>();
        si.uv = position_sample;
        si.wavelengths = wavelengths;
        return std::make_pair(ray, wav_weight * code->eval(si, active));
    }
    ...
    ScalarBoundingBox3f bbox() const override {
        Throw("Called?");
        ScalarBoundingBox3f bb = m_world_transform->translation_bounds();
        bb.min *= m_scale;
        bb.max *= m_scale;
        return bb;
    }
    ...
    std::string to_string() const override {
        ...
        oss << "CodedOrthographicCamera[" << std::endl
...
MTS_IMPLEMENT_CLASS_VARIANT(CodedOrthographicCamera, ProjectiveCamera)
MTS_EXPORT_PLUGIN(CodedOrthographicCamera, "Coded Perspective Camera");
